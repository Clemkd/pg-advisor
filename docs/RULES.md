...
- comme **paramètre SQL** dans `query`, sous la forme `@ratio_minimum` ;
...
- cast de style PostgreSQL : `used::float`, `valeur::int`, `valeur::text`, `valeur::bool`
...
- consulter son applicabilité instance par instance, avec le motif du refus ;
- créer, dupliquer ou supprimer une règle utilisateur ;
- poser une surcharge globale ou par instance : activation, sévérité, périodicité, seuils.
...
pour une instance. L'ordre de précédence est : valeur du fichier, puis surcharge globale, puis
surcharge d'instance. Une surcharge de seuils ne remplace que les seuils cités ; les autres
gardent la valeur du fichier.
...
| `GET /api/rules/{id}` | Règle, YAML et applicabilité par instance |
